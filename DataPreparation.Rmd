...
library(lubridate), library(zoo), library(imputeTS), library(DataExplorer), library(data.table), library(DT) 
...
colSums(is.na(data))
```
The variable Duration has missing values. Since only a small number of observations have missing values, it was decided that the missing values will be replace by the mean value
```{r}
...
The variable infants has a maximum value of 255, this is likely to be an error based on the mean and median. Furthermore it is unlikely to have 255 infants in a holiday.To verify the error a scatter plot is used to get a better understanding.

```{r, }
plot(data$Infants)
```
Based on the plot, the row with 255 infants was eliminated as this is definitely a data entry error.
```{r}
outlierReplace(data,"Infants",which(data$Infants>10),NA)
```
\newpage
Checking the plot again for Infants
```{r}
plot(data$Infants)
```

From the plot it is observed that there is only two observations where Infants >1, hence it was decided that these observations will be treated as outliers.
```{r}
outlierReplace(data,"Infants",which(data$Infants>1),NA)
```
\newpage
...
From the plot, it is observed that these is some extreme values, it is ideal to remove these extreme values to prevent overfitting. In this case, it is decided that all observations with greater than 10 adults will be treated as outliers. Sometimes it is ideal to use a box-plot to eliminate outliers, but in this dataset a boxplot method to remove outliers removed many important observations. Hence, the outlier function created earlier was used to deal with the outliers.

```{r}
outlierReplace(data,"Adults",which(data$Adults>10),NA)
```
\newpage
Outliers in Duration was treated using the same method
```{r}
plot(data$Duration)
outlierReplace(data,"Duration",which(data$Duration>21 & data$Duration<5),NA)
plot(data$Duration)
```


A plot of children was created to understand the outliers in the varaiable children
```{r}
plot(data$Children)
```
...
outliers2 <- boxplot(data$Children, plot=FALSE)$out
data <- data[-which(data$Children %in% outliers2),]
```

Similarly outliers in Lead.Time was treated using the same method
```{r}
boxplot(data$Lead.Time)
outliers3 <- boxplot(data$Lead.Time, plot=FALSE)$out
data <- data[-which(data$Lead.Time %in% outliers3),]
```
